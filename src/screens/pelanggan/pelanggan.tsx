...
export default function Pelanggan({ navigation }: Props) {
  const [data, setData] = useState<PelangganProps[]>([]);
  ...
  const renderItem = ({ item }: { item: PelangganProps }) => (
    <StyledTouchableOpacity className="max-w-sm w-full mt-6 lg:max-w-full lg:flex bg-white border border-gray-400 rounded-lg overflow-hidden">
      ...
      <StyledView className="h-48 lg:h-auto lg:w-48 flex-none bg-cover rounded-t lg:rounded-t-none lg:rounded-l text-center overflow-hidden">
        <StyledImage
          source={{
            uri: "https://cdn-icons-png.freepik.com/512/7718/7718888.png",
          }}
            ...
            <StyledView className="flex flex-row ">
              <StyledView className="mr-2">
                <StyledButton
                  title="Edit"
                  onPress={() =>
                    navigation.navigate("EditPelanggan", { id: item.id })
                  }
                />
              </StyledView>
  ...
  const renderFooter = () => (
    <StyledView className="py-5">
      <StyledButton
        ...
        onPress={() => navigation.navigate("AddPelanggan")} // Pindah ke AddBarang
      />
    </StyledView>
  );
  return (
